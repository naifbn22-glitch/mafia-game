...
let activeRoomViewCleanup = null;
let activeRefreshFrame = null;

function bindRealtimeViewRefresh(refresh) {
  activeRoomViewCleanup?.();

  const scheduleRefresh = () => {
    if (activeRefreshFrame !== null) return;
    activeRefreshFrame = window.requestAnimationFrame(() => {
      activeRefreshFrame = null;
      refresh();
    });
  };

  window.addEventListener("mafia-rooms-updated", scheduleRefresh);
  channel?.addEventListener("message", scheduleRefresh);

  activeRoomViewCleanup = () => {
    window.removeEventListener("mafia-rooms-updated", scheduleRefresh);
    channel?.removeEventListener("message", scheduleRefresh);
    if (activeRefreshFrame !== null) {
      window.cancelAnimationFrame(activeRefreshFrame);
      activeRefreshFrame = null;
    }
  };
}
